...
% Recover optimal D from Lemma 1: d_i* = (U*Y)_i(U*X)_i* / ||(U*X)_i||^2
UX = U' * X;
UY = U' * Y;
d = zeros(m, 1);
for i = 1:m
    phi_i = norm(UX(i,:))^2;
    if phi_i > 0
        d(i) = UY(i,:) * UX(i,:)' / phi_i;  % d_i* = (U*Y)_i(U*X)_i* / phi_i
    end
end
D = diag(d);
A = U * D * U';
...
function G = euclidean_grad(U, X, Y)
    ...
    G = 2 * (X*Y'*U*Dstar + Y*X'*U*Dstar_conj - X*X'*U*Dstar_abs2);
end
...
function H = euclidean_hess(U, W, X, Y, M)
    ...
    for i = 1:m
        phi(i) = norm(UX(i,:))^2;
        if phi(i) > 0
            lambda(i) = UY(i,:) * UX(i,:)';      % lambda_i = (U*Y)_i(U*X)_i*
            d(i)      = lambda(i) / phi(i);       % d_i* = lambda_i / phi_i
        end
    end
    ...
    % delta(d_i*) = delta(lambda_i)/phi_i - lambda_i/phi_i^2 * delta(phi_i)
    delta_d = zeros(m, 1);
    for i = 1:m
        if phi(i) > 0
            delta_d(i) = delta_lambda(i) / phi(i) ...
                       - lambda(i) / phi(i)^2 * delta_phi(i);
        end
    end
    ...
    H = 2 * ( ...
        X*Y'*W*Dstar       + X*Y'*U*dDstar       + ...
        Y*X'*W*Dstar_conj  + Y*X'*U*dDstar_conj  - ...
